...
class DataProfiler:
        ...
        if len(non_null) > 0:
            ...
            if pd.api.types.is_numeric_dtype(series):
                ...
                if col_profile["unique_count"] <= MAX_UNIQUE_VALUES_DISPLAY:
                    col_profile["unique_values"] = non_null.unique().tolist()[
                        :MAX_UNIQUE_VALUES_DISPLAY
                    ]
                else:
    ...
    def _detect_column_type(self, series: pd.Series) -> str:
        ...
        if pd.api.types.is_numeric_dtype(series):
            if (
                "amount" in series.name.lower()
                or "value" in series.name.lower()
                or "price" in series.name.lower()
            ):
                return "monetary"
    ...
    def _looks_like_date(self, sample: pd.Series) -> bool:
        ...
        try:
            ...
            if self.strict_date_detection and self._is_numeric_id_pattern(sample):
                logger.debug(
                    f"Strict date detection: rejected numeric ID pattern for {sample.name}"
                )
                return False
    ...
    def _is_numeric_id_pattern(self, sample: pd.Series) -> bool:
        ...
        try:
            ...
            avg_non_numeric_ratio = (
                non_numeric_chars.str.len().mean() / sample.astype(str).str.len().mean()
            )
    ...
    def _calculate_quality_score(self, columns: dict[str, Any]) -> float:
        ...
        for col_info in columns.values():
            ...
            if col_info.get("detected_type") == "id":
                ...
                uniqueness_score = 1 - abs(
                    UNIQUENESS_SWEET_SPOT - uniqueness
                )  # Sweet spot around 50%

            col_score = (
                1 - null_penalty
            ) * QUALITY_SCORE_NULL_WEIGHT + uniqueness_score * QUALITY_SCORE_UNIQUENESS_WEIGHT
    ...
    def generate_expectations(
        ...
        null_tolerances: dict[str, float] | None = None,
        ...
        for col, col_info in self.profile_results["columns"].items():
            ...
            if include_completeness:
                expectations.extend(
                    self._generate_completeness_expectations(
                        col, col_info, col_null_tolerance, severity_threshold
                    )
                )
            ...
            expectations.append(
                {
                    ...
                    "meta": {
                        "severity": severity,
                        "description": "Values should be one of the known categories",
                    },
                }
            )
            ...
            expectations.append(
                {
                    ...
                    "meta": {
                        "severity": "low",
                        "description": "String length should be within observed range",
                    },
                }
            )
